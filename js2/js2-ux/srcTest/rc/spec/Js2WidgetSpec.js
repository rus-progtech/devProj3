...
define([
  ...
  'nmodule/js2/rc/Js2Widget' ], function (
  baja,
  $,
  Promise,
  promiseUtils,
  ...
  describe('nmodule/js2/rc/Js2Widget', () => {
    let stooges,
        widget,
        elem;
    ...
    describe('#doInitialize()', () => {
      it('creates the structure to load buttons into', () => {
        expect(elem.find('.Js2Widget-header').length).toBe(1);
        expect(elem.find('.Js2Widget-content').length).toBe(1);
      });

      it('arms a handler to set active class on button click', () => {
        var contentDom = elem.find('.Js2Widget-content'),
      ...
      it("removes active class from other buttons when one is clicked", () => {
        const contentDom = elem.find('.Js2Widget-content');
      ...
      it('arms a handler to display selected slot name', () => {
        return widget.load(stooges)
          .then(function () {
            const slotDom = elem.find('.Js2Widget-selected-slot');
            const contentDom = elem.find('.Js2Widget-content');
